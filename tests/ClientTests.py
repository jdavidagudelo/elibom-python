...
def mock_get(url, data=None, **kwargs):
    if url.endswith('messages/123'):
        ...
        response.content = json.dumps(
            {'id': 456, 'status': 'scheduled', 'isFile': True, 'fileName': 'test.xls', 'fileHasText': False,
             'text': 'test'})
        return response
        ...
        response.ok = True
        response.content = json.dumps(
            [{'id': 456, 'isFile': True, 'fileName': 'test.xls', 'fileHasText': False, 'text': 'test'},
             {'id': 457, 'isFile': True, 'fileName': 'test.xls', 'fileHasText': False, 'text': 'test'}])
        return response
        ...
        response.ok = True
        response.content = json.dumps([{'id': 1, 'name': 'testUser', 'email': 'test@dominio.com', 'status': 'active'},
                                       {'id': 2, 'name': 'testUser2', 'email': 'test2@dominio.com',
                                        'status': 'active'}])
        return response
...
class ElibomClientTests(unittest.TestCase):
    ...
    def test_should_schedule_message(self, mock_requests):
        mock_requests.post = mock_post
        elibom = ElibomClient('user@domain.com', 'password')
        scheduleId = elibom.schedule_message('573017897304', 'test message', '2018-09-27 23:00')
