...
class ElibomClient(object):
    ...
    def send_message(self, destination, text):
        ...
        if response.ok:
            ...
            content = getattr(content, 'decode', lambda d: content)('utf-8')
            response_content = json.loads(content, encoding='utf-8')
    ...
    def schedule_message(self, destination, text, scheduled_date):
        ...
        """
        if not self.__is_valid_destination(destination):
            raise ElibomClientException(
                'Invalid destination, cannot be None, and must be composed of a least 10 digits.')
        if not self.__is_valid_text(text):
            raise ElibomClientException('Invalid text, cannot be None, and must not be empty')
    ...
    def cancel_schedule(self, scheduleId):
        ...
        Attributes:
            scheduleId: the schedule identifier.
        """
        response = requests.delete(self.__api_base_url + 'schedules/' + str(scheduleId),
                                   auth=(self.user, self.password), headers=self.__headers)
        if response.ok:
            return scheduleId
        else:
            self.__manage_error_response(response)
